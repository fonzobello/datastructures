...
public class SearchHeuristicAStar<V, E, R> {
	...
	protected HashMap<Vertex<V>, Double> _FScore;
	...
	public R execute(Graph<V, E> graph, Vertex<V> startVertex, Vertex<V> endVertex) {
		...
		//Initialize Collections
			...
			_FScore = new HashMap<Vertex<V>, Double>();
		...
		//Initialize a priority queue of paths with the one-node path consisting of the initial state
			...
			_FScore.put(_Start, _GScore.get(_Start) + _HScore.get(_Start));
			
			_OpenSet.insert(_FScore.get(_Start), _Start);
			...
			//Else extend the path by one node in all possible ways,
				...
				for (Edge<E> edge: _Graph.incidentEdges(vertex)) {
					...
					if (!_OpenSet.contains(oppositeVertex)) {
						...
						_FScore.put(oppositeVertex, _GScore.get(oppositeVertex) + _HScore.get(oppositeVertex));
						
						_OpenSet.insert(_FScore.get(oppositeVertex), oppositeVertex);
