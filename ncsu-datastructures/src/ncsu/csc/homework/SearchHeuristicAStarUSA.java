...
import ncsu.csc.search.SearchHeuristicAStar;

public class SearchHeuristicAStarUSA extends SearchHeuristicAStar<City, Integer, List<Vertex<City>>> {

	public LinkedList<Vertex<City>> _Expanded;
	
	public SearchHeuristicAStarUSA() {
	...
	public List<Vertex<City>> execute(Graph<City, Integer> graph, Vertex<City> startVertex, Vertex<City> endVertex) {
		
		List<Vertex<City>> toReturn = super.execute(graph, startVertex, endVertex);
		
		System.out.println("Expanded Nodes: " + _Expanded);
		
		System.out.println("Expanded Node Count: " + _Expanded.size());
		
		System.out.println("Solution Nodes: " + toReturn);
		
		System.out.println("Solution Node Count: " + toReturn.size());
	...
	protected Double heuristic(Vertex<City> current, Vertex<City> goal) {
		...
		Double distanceX = deltaLongitude * 69.5 * Math.abs(Math.cos(Math.toRadians((current.element().getLatitude() + goal.element().getLatitude()) / 2.0)));
